...
const isPublicOptions: {
...
}[] = [
  {
    value: true,
    label: "공개",
    className: "rounded-full bg-blue-100 text-blue-500!",
  },
  {
    value: false,
    label: "비공개",
...
const statusOptions: {
  ...
  },
  {
    value: "APPROVED",
    label: "승인",
    className: "rounded-full bg-green-100 text-green-500!",
  },
  {
    value: "CANCEL",
    label: "취소",
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  try {
    const myTributes = await tributeService.get.tributeList({
      userId: user.id,
      token,
    });
...
export default function Mypage({
  ...
  const [marketingAgreed, setMarketingAgreed] = useState<"true" | "false">(
    (String(user.marketingAgreed) as "true" | "false") || "false",
  );
  const [selectedTributeId, setSelectedTributeId] = useState<string>("");
  const [tributeIsOpen, setTributeIsOpen] = useState<boolean>(false);
  const [tributeDeleteIsOpen, setTributeDeleteIsOpen] =
  ...
  const [deleteMemorialIsOpen, setDeleteMemorialIsOpen] =
    useState<boolean>(false);
  ...
  const selectedMemorial = myMemorials.find(
    (memorial) => memorial.id === selectedMemorialId,
  );
  ...
  const memorialReadOnlyFields = selectedMemorial
    ? [
        ...
        {
          label: "등록일",
          value: moment(selectedMemorial.createdAt).format("YYYY-MM-DD HH:mm"),
        },
        {
          label: "수정일",
          value: moment(selectedMemorial.updatedAt).format("YYYY-MM-DD HH:mm"),
        },
      ]
    : [];
  ...
  const handleChangeTributeContent = (
    e: React.ChangeEvent<HTMLTextAreaElement>,
  ) => {
    setTributeForm((prev) => ({ ...prev, content: e.target.value }));
  };

  const handleToggleTributeEdit = async (
  ...
  const handleChangeMemorialField =
    (field: keyof typeof memorialForm) =>
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setMemorialForm((prev) => ({ ...prev, [field]: e.target.value }));
    };

  const handleToggleMemorialEdit = async (
    e: React.MouseEvent<HTMLButtonElement>,
  ) => {
    e.preventDefault();
    if (isMemorialEdit) {
      ...
      <FlexDiv className="flex-col gap-y-6">
              ...
              <FieldGroup>
                ...
                <Field>
                  ...
                  <FlexDiv className="flex gap-x-2 items-center">
                    <Switch id="marketingAgreed" disabled={!isEdit} />
        ...
        <Card>
          ...
          <CardContent>
                ...
                {selectedTributeId && (
                  <div className="space-y-4">
                    <FieldGroup>
                      ...
                      <Field>
                        <FieldLabel htmlFor="tribute-visibility">
                          공개 여부
                        </FieldLabel>
                        <Combobox
                          ...
                          value={
                            isPublicOptions.find(
                              (option) => option.value === tributeForm.isPublic,
                            ) ?? null
                          }
                          onValueChange={(option) =>
                            setTributeForm((prev) => ({
                              ...prev,
                              visibility: option?.value ?? false,
                            }))
                          }
                          disabled={!isTributeEdit}
                        >
                          <ComboboxInput
                            id="tribute-visibility"
            ...
            <Dialog
              open={tributeDeleteIsOpen}
              onOpenChange={handleTributeDeleteOpenChange}
            >
              <DialogContent>
                <DialogHeader>
        ...
        <Card>
          ...
          <CardContent>
              ...
              {myMemorials.map((memorial) => (
                ...
                >
                  <div className="aspect-square relative overflow-hidden bg-gray-200">
                    <img
                      src={
                        memorial.photoUrl || "https://placehold.co/380x380/png"
                      }
                      alt="추모관 사진"
            ...
            <Dialog
              open={memorialIsOpen}
              onOpenChange={handleMemorialOpenChange}
            >
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>
                    {selectedMemorial?.deceasedName} 추모관
                  </DialogTitle>
                </DialogHeader>
                {selectedMemorial && (
                  <div className="space-y-4">
                    <div className="aspect-square w-32 mx-auto overflow-hidden bg-gray-200">
                      <img
                        src={
                          memorialForm.photoUrl ||
                          "https://placehold.co/380x380/png"
                        }
                        alt="추모관 사진"
                        className="w-full h-full object-cover"
                      />
                    ...
                    <FieldGroup>
                        ...
                        <Combobox
                          ...
                          value={
                            visibilityOptions.find(
                              (option) =>
                                option.value === memorialForm.visibility,
                            ) ?? null
                          }
                          onValueChange={(option) =>
                      ...
                      <Field className="w-fit">
                        ...
                        <Badge
                          className={
                            statusOptions.find(
                              (option) => option.value === memorialForm.status,
                            )?.className
                          }
                        >
                          {statusOptions.find(
                            (option) => option.value === memorialForm.status,
                          )?.label ?? memorialForm.status}
                        </Badge>
                      </Field>
                    </FieldGroup>
            ...
            <Dialog
              open={deleteMemorialIsOpen}
              onOpenChange={setDeleteMemorialIsOpen}
            >
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>추모관 삭제</DialogTitle>
                </DialogHeader>
                정말로 추모관을 삭제하시겠습니까?
                <DialogFooter>
                  <Button
